...
class Lunar @JvmOverloads constructor(cal: Calendar = Calendar.getInstance()) {
    private val year // 农历年
            : Int
    private val month // 农历月
            : Int
    private val day // 农历日
            : Int
    private var leap // 农历闰年
            : Boolean
     ...
     * 构造一个表示指定日期的农历日历
     */
    constructor(date: Date) : this(date2calendar(date))
    /**
     * 使用指定日历日期构造一个农历日历
     *
     * @param cal
     ...
     */
    init {
        @Suppress("unused") val yearCyl: Int
        @Suppress("unused") var monCyl: Int
        @Suppress("unused") val dayCyl: Int
        var leapMonth = 0
    ...
    override fun toString(): String {
        return year.toString() + "年" + (if (leap) "闰" else "") + chineseNumber[month - 1] + "月" + getChinaDayString(
            day
        )
    }

    val lunarMonth: Int = month
    val lunarDay: Int = day
    ...
    companion object {
        ...
        private fun date2calendar(date: Date): Calendar {
            val tmpCale = Calendar.getInstance()
            tmpCale.time = date
            return tmpCale
        }
